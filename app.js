...
window.openDetail=id=>{const c=(window.__cards||[]).find(x=>String(x.id)===String(id));if(!c)return;document.getElementById("detailModal").innerHTML=`<div class="sheet"><div class="modal-head"><h3>${tr("detailTitle")}</h3><button class="x" onclick="detailModal.classList.remove('show')">×</button></div><div class="detail-grid"><div class="detail"><b>${tr("bank")}</b>${esc(c.bank)}</div><div class="detail"><b>${tr("cardNumber")}</b>•••• •••• •••• ${esc(c.last4)}</div><div class="detail"><b>${tr("cvv")}</b><span style="display:inline-flex;align-items:center;gap:8px"><span id="detailCvv" style="font-family:monospace" data-v="${esc(c.cvv||"—")}">•••</span><button type="button" id="detailCvvToggle" style="border:1px solid #333;background:#181818;color:#fff;border-radius:8px;padding:4px 10px;font-size:12px;cursor:pointer">${tr("show")}</button></span></div><div class="detail"><b>${tr("network")}</b>${esc(c.network||"")}</div><div class="detail"><b>${tr("expiry")}</b>${esc(c.expiry)}</div><div class="detail"><b>${tr("statusLabel")}</b>${c.status==="valid"?tr("valid"):tr("invalid")}</div>${c.status==="valid"&&!c.is_default?`<button class="primary" onclick="setDefault('${c.id}')">${tr("setDefault")}</button>`:""}</div></div>`;detailModal.classList.add("show");
 const cvvEl=document.getElementById("detailCvv");
 const cvvBtn=document.getElementById("detailCvvToggle");
 if(cvvEl&&cvvBtn){cvvBtn.onclick=()=>{const v=cvvEl.dataset.v||"—";const shown=cvvEl.dataset.shown==="1";if(shown){cvvEl.textContent="•••";cvvEl.dataset.shown="0";cvvBtn.textContent=tr("show")}else{cvvEl.textContent=v;cvvEl.dataset.shown="1";cvvBtn.textContent=tr("hide")}};}
};
